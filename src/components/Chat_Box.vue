...
export default {
	...
	watch: {
		localUser(newValue) {
			...
			this.createBubbleFront(this.welcomeMsg, 'bubFront');
			this.displayChat();
		},
	},
	...
	methods: {
		...
		createBubbleSOcket(message, oneClass) {
			const bubble = document.createElement('div');
			bubble.classList.add(oneClass);

			const p = document.createElement('p');
			p.textContent = this.msgBody.msgToSend;
		...
		createBubbleFront(message, oneClass) {
			const bubble = document.createElement('div');
			bubble.classList.add(oneClass);
